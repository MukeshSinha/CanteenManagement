...
const EmployeeRawPunch: React.FC = () => {
    const [fromDate, setFromDate] = useState<string>('');
    const [upToDate, setUpToDate] = useState<string>('');

    const allOption: EmployeeOption = { empCode: 'ALL', empName: 'All Employees', label: 'ALL - All Employees' };

    // Autocomplete states
    const [employeeOptions, setEmployeeOptions] = useState<EmployeeOption[]>([allOption]);
    const [selectedEmployee, setSelectedEmployee] = useState<EmployeeOption | null>(allOption);
    ...
    const handleShowReport = async () => {
        ...
        if (!selectedEmployee || !selectedEmployee.empCode) {
            Toast.fire({
                icon: 'warning',
                title: 'Please select or type an Employee Code.'
            });
            return;
        }
        ...
        try {
            ...
            if (selectedEmployee.empCode !== 'ALL') {
                params.empcode = selectedEmployee.empCode;
            }
            const queryString = new URLSearchParams(params).toString();

            const url = `Employee-RawPunch/get-employee-raw-punch?${queryString}`;
    ...
    const exportToCSV = () => {
        ...
        saveAs(blob, `EmployeeRawPunch_${selectedEmployee?.empCode || 'Report'}_${fromDate}_to_${upToDate}.csv`);
    };
    ...
    const exportToExcel = () => {
        ...
        saveAs(blob, `EmployeeRawPunch_${selectedEmployee?.empCode || 'Report'}_${fromDate}_to_${upToDate}.xlsx`);
    };

    return (
        <Box sx={{ p: 3, bgcolor: '#f5f7fa', minHeight: '100vh' }}>
            <Typography variant="h4" fontWeight={600} gutterBottom sx={{ color: '#1976d2', mb: 3 }}>
                Employee Raw Punch Report
            </Typography>

            <Card sx={{ mb: 4, borderRadius: 3, boxShadow: '0 4px 20px rgba(0,0,0,0.05)' }}>
                <CardContent sx={{ p: 3 }}>
                    {/* Row 1: Filters (From Date, Upto Date, Employee Autocomplete) */}
...
export default EmployeeRawPunch;
